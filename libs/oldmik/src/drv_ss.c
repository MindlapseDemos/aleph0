...
static BOOL SS_Init(void)
{
	...
	if(!SS_IsThere()){
		myerr="No such hardware detected, check your 'SNDSCAPE' env. variable";
		return 0;
	}
	...
	SS_DMABUF=(char *)MDma_GetPtr(SS_DMAMEM);
...
static void SS_PlayStart(void)
{
	...
	if(!MDma_Start(DmaChan,SS_DMAMEM,md_dmabufsize,INDEF_WRITE)){
		return;
	}
...
DRIVER drv_ss={
	NULL,
	"Ensoniq Soundscape",
	"MikMod Ensoniq Soundscape Driver v0.0 - Thanks to CyberCerus",
	SS_IsThere,
	VC_SampleLoad,
	VC_SampleUnload,
	SS_Init,
