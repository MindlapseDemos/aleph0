...
static SLONG voltab[65][256];
static UWORD volsel[65];
...
void AsmMonoNormal(SBYTE *srce,SLONG *dest,SLONG index,SLONG increment,ULONG todo);
#pragma aux AsmMonoNormal     \
        parm [esi] [edi] [ebx] [ecx] [edx] \
        modify [eax];
...
void freedescriptor(unsigned short selector);
#pragma aux freedescriptor =    \
	"mov    ax,0001h"       \
	"int    31h"            \
	parm    [bx]            \
	modify  [ax];

unsigned short getalias(void);
#pragma aux getalias =          \
	"mov    ax,cs   "       \
	"mov    bx,ax   "       \
	"mov    ax,000ah"       \
...
void setbase(unsigned short selector,unsigned long offset);
#pragma aux setbase =           \
	"mov    ax,0007h"       \
	"mov    ecx,edx"        \
	"ror    ecx,16"         \
...
void VC_Sample32To16Copy(SLONG *srce,SWORD *dest,ULONG count,UBYTE shift);
#pragma aux VC_Sample32To16Copy =	\
"again:"					\
	"mov   eax,[esi]"		\
	"sar   eax,cl"          \
	"cmp   eax,32767"		\
...
void VC_Sample32To8Copy(SLONG *srce,SBYTE *dest,ULONG count,UBYTE shift);
#pragma aux VC_Sample32To8Copy =	\
"again:"                   	\
	"mov   eax,[esi]"	   	\
	"sar   eax,cl"      	\
	"cmp   eax,127"     	\
			...
			for(t=0;t<md_numchn;t++){
				...
				if(vinf[t].active){
					...
					if(md_mode & DMODE_STEREO){
						...
						rvol= ( vol * pan ) / 255;
						vinf[t].lvolsel=volsel[lvol];
						vinf[t].rvolsel=volsel[rvol];
					}
					else{
						vinf[t].lvolsel=volsel[vol];
					}
#else
					if(md_mode & DMODE_STEREO){
						lvol= ( vol * (255-pan) ) / 255;
